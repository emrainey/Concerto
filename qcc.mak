...
$(_MODULE)_COPT := -fPIC -Wno-write-strings
ifdef DEBUG 
...
$(_MODULE)_AFLAGS   := $($(_MODULE)_INCLUDES) -meabi=5 --gdwarf-2
...
$(_MODULE)_UNLN_DSO   := $(CLEAN) $($(_MODULE)_INSTALL_PATH)/usr/lib/$($(_MODULE)_BIN)
$(_MODULE)_INSTALL_DSO:= install -C -p -m 755 $($(_MODULE)_BIN) $($(_MODULE)_INSTALL_PATH)/usr/lib/
$(_MODULE)_UNINSTALL_DSO:=$(CLEAN) $($(_MODULE)_INSTALL_PATH)/usr/lib/$($(_MODULE)_BIN)
$(_MODULE)_INSTALL_EXE:= install -C -p -m 755 $($(_MODULE)_BIN) $($(_MODULE)_INSTALL_PATH)/usr/bin/
$(_MODULE)_UNINSTALL_EXE:=$(CLEAN) $($(_MODULE)_INSTALL_PATH)/usr/bin/$($(_MODULE)_BIN)
...
$(_MODULE)_LINK_DSO   := $(LD) -shared --export-dynamic -soname,$($(_MODULE)_BIN).1 -whole-archive $($(_MODULE)_LIBRARIES) -no-whole-archive -o $($(_MODULE)_BIN).1.0 $($(_MODULE)_OBJS) $($(_MODULE)_MAP)
